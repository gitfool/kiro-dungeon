...
# sync-dotnet-test-migration.sh — Mirrors the dotnet/skills dotnet-test-migration
# plugin skills and orchestrator agent from GitHub into this power's skills/
# directory.
#
# Downloads the dotnet-test-migration plugin from the dotnet/skills repo and
# mirrors its skills into ./skills/, so this directory can be installed as a Kiro
# power.
#
# Additionally, the plugin's single orchestrator agent (agents/*.agent.md) is
# converted to a skill. Kiro does not consume the Agent Plugins "agents" manifest
# field, so the agent content would otherwise be dropped. The test-migration agent
# is a user-facing entry point (user-invokable: true) that auto-detects the
# framework/version/platform and routes to the right migration skill — substantial
# standalone content, not an internal pipeline stub — so it is converted.
# Conversion strips agent-only frontmatter (user-invokable/user-invocable, tools,
# and the multi-line handoffs: / agents: blocks that reference other agents),
# normalizes the name to the skill directory name, and keeps the rest so the
# skill's activation intent is preserved. disable-model-invocation is kept
# verbatim: Kiro ignores it today (see issue #10985 below), so preserving it is
# harmless now and retains the author's intent for when Kiro honors it. The agent
# name is already descriptive, so no rename is needed.
#
# The agent carries editor/host tool references (task, web_fetch, web_search,
# skill) that mean nothing on Kiro. Rather than rewrite the prose, the sync
# appends a short "Kiro tool interpretation" note to each converted agent skill,
# mapping those references to Kiro equivalents at read time. Unlike dotnet-msbuild,
# dotnet-test-migration ships NO MCP server, so the note makes no mention of one.
#
# CROSS-POWER REFERENCES: both the migration skills and the converted agent
# reference skills/agents that live in the sibling dotnet-test power, not here:
# platform-detection, writing-mstest-tests, run-tests (skills), and
# test-quality-auditor / testability-migration (agents, themselves converted to
# skills in dotnet-test). Install the dotnet-test power alongside this one to get
# the full workflow. The Kiro note and README say so rather than implying those
# names are siblings in this power.
#
# The manifest (plugin.json), POWER.md, README, and this script are maintained by
# hand and are NOT touched by the sync (except the version field in plugin.json
# and POWER.md, which write_power_version stamps) — it only writes under skills/.
# Upstream's plugin.json is intentionally NOT mirrored: it is non-compliant with
# Agent Plugins 1.0.0 (missing $schema, and carries skills/agents keys Kiro
# strips). See https://github.com/dotnet/skills/issues/1087. The upstream
# README.md, .claude-plugin/, .codex-plugin/, and version.json live outside
# skills/ and agents/, so they are naturally excluded.
#
# Support directories (references, scripts) under a skill are mirrored as-is.
# Only immediate subdirectories of skills/ are treated as skills by Kiro, so
# nested markdown is never loaded and needs no special handling.
#
# This is a maintainer tool, not an installer. Run it to refresh skills/ when
# upstream changes, then commit the result.
#
# Usage:
#   sync-dotnet-test-migration.sh [--dry-run] [--ref <branch|tag|sha>]
...
BASE_PATH="plugins/dotnet-test-migration/skills"
AGENTS_PATH="plugins/dotnet-test-migration/agents"
# Path within the upstream repo whose commit date drives this power's version.
# The whole plugin dir, not just skills/, so a change to the converted agent or
# other plugin content is still reflected in the version.
UPSTREAM_PATH="plugins/dotnet-test-migration"
...
# dotnet-test-migration agent is a user-facing orchestrator (user-invokable:true),
# substantial standalone content, so it is converted.
AGENTS_TO_CONVERT=(test-migration)

# Agent -> skill directory renames. None needed: test-migration is already a
# descriptive, specific name that sits fine alongside the migrate-* skills.
...
echo -e "${GREEN}sync-dotnet-test-migration: $REPOSITORY@$REF ($BASE_PATH)${RESET}"
...
# Strips agent-only frontmatter and normalizes the name to the skill directory
# name, then appends a Kiro tool-interpretation note after the body.
#
# The stripped keys include multi-line YAML blocks (handoffs:, agents:) whose
# nested list items are indented continuation lines. The awk drops a stripped
# key and any following indented (or blank) continuation lines until the next
# top-level (column-0) key or the closing '---'. This deliberately preserves the
# multi-line `description: >-` block scalar, which is NOT a stripped key.
#
# disable-model-invocation is deliberately NOT stripped. Kiro currently ignores
# it (a silent no-op, verified in https://github.com/kirodotdev/Kiro/issues/10985),
# so keeping it is harmless today and preserves the upstream author's explicit-
# invocation intent verbatim for the day Kiro honors the key. Stripping it would
# silently discard a real constraint if upstream ever flips it to true, and the
# sync would surface no diff to flag the loss. See the dungeon playbook §4.
convert_agent_to_skill() {
    ...
    awk -v name="$skill_name" '
        BEGIN { fm = 0; skip = 0 }
        # Frontmatter fences. A top-level line ends any active skip block.
        /^---[[:space:]]*$/ { skip = 0; fm++; print; next }
        # Outside the frontmatter, copy through unchanged.
        fm != 1 { print; next }
        # Inside frontmatter: a top-level key (no leading whitespace) either
        # starts a strip block, is the normalized name, or is kept as-is.
        /^[A-Za-z0-9_-]+:/ {
            skip = 0
            if ($0 ~ /^name:/) { print "name: " name; next }
            if ($0 ~ /^(user-invokable|user-invocable|tools|handoffs|agents):/) {
                skip = 1
                next
            }
            print
        ...
        # Continuation lines (indented or blank): drop while skipping a block,
        # otherwise keep (e.g. the description block scalar continuation).
        { if (skip) next; print }
...
- `task` / `#tool:agent/runSubagent` / `runSubagent` → dispatch a Kiro sub-agent
  with `invoke_sub_agent` (e.g. the `general-task-execution` or `context-gatherer`
...
This power ships no MCP server. The migration skills it routes to
(`migrate-mstest-v1v2-to-v3`, `migrate-xunit-to-mstest`, etc.) are siblings in
this same power and load on description match or via `disclose_context`. Some
references, however, live in the sibling **dotnet-test** power, not here:
`platform-detection`, `writing-mstest-tests`, and `run-tests` (skills), plus any
`test-quality-auditor` / `testability-migration` handoff (agents that dotnet-test
converts to skills). Install the dotnet-test power alongside this one to resolve
them; without it, apply the detection/verification guidance inline from your own
knowledge.
EOF
    fi
}
...
# tag (e.g. v0.1.9) or the default branch (the bump workflow's path), so there is
# one source of truth. The regression guard refuses a version that would move
