...
# sync-dotnet-test.sh — Mirrors the dotnet/skills dotnet-test plugin skills and
# selected agents from GitHub into this power's skills/ directory.
#
# Downloads the dotnet-test plugin from the dotnet/skills repo and mirrors its
# skills into ./skills/, so this directory can be installed as a Kiro power.
#
# Additionally, the two user-facing agent definitions (agents/*.agent.md) are
# converted to skills. Kiro does not consume the Agent Plugins "agents" manifest
# field, so the agent content would otherwise be dropped. Conversion strips
# agent-only frontmatter (user-invokable/user-invocable, tools, and the
# multi-line handoffs: / agents: blocks that reference other agents), normalizes
# the name to the skill directory name, and keeps the rest so the skill's
# activation intent is preserved. disable-model-invocation is kept verbatim: Kiro
# ignores it today (see issue #10985 below), so preserving it is harmless now and
# retains the author's intent for when Kiro honors it.
#
# Only the two user-facing agents (test-quality-auditor, testability-migration)
# are converted. The eight internal pipeline subagents
# (code-testing-generator/-researcher/-planner/-implementer/-builder/-tester/
# -fixer/-linter) are user-invocable:false stages of a nested GitHub Copilot /
# VS Code fan-out swarm that Kiro cannot reproduce natively; they are skipped
# (the analog of pstack's poteto-agent routing stub). The public entry to that
# pipeline is the code-testing-agent SKILL, which is mirrored like any other
# skill. Both converted agents already have descriptive, specific names, so —
# unlike the sibling dotnet-msbuild power's "msbuild" agent — no rename is needed.
#
# The agents carry editor/host tool references (task, web_fetch, web_search,
...
# mapping those references to Kiro equivalents at read time. The note is injected
# into all converted agents (harmless for those that don't use the tools,
# future-proof if upstream adds such references later). Unlike dotnet-msbuild,
# dotnet-test ships NO MCP server, so the note makes no mention of one.
#
...
# OVERLAYS.md, README.md, .claude-plugin/, .codex-plugin/, and version.json live
# outside skills/ and agents/, so they are naturally excluded.
#
...
#
# Usage:
#   sync-dotnet-test.sh [--dry-run] [--ref <branch|tag|sha>]
...
BASE_PATH="plugins/dotnet-test/skills"
AGENTS_PATH="plugins/dotnet-test/agents"
# Path within the upstream repo whose commit date drives this power's version.
# The whole plugin dir, not just skills/, so a change to a converted agent or
# other plugin content is still reflected in the version.
UPSTREAM_PATH="plugins/dotnet-test"
# Default ref when --ref is not given (the bump workflow's path).
REF="${REF:-main}"

# Agent basenames (without the .agent.md suffix) to convert to skills. Only the
# two user-facing entry-point agents are converted; the eight internal
# user-invocable:false pipeline subagents are skipped (Kiro cannot reproduce the
# nested Copilot/VS Code sub-agent fan-out they form).
AGENTS_TO_CONVERT=(test-quality-auditor testability-migration)

# Agent -> skill directory renames. None needed: both converted agents already
# have descriptive, specific names that sit fine alongside the other skills.
...
echo -e "${GREEN}sync-dotnet-test: $REPOSITORY@$REF ($BASE_PATH)${RESET}"
...
This power ships no MCP server. Any agent "handoffs" or nested sub-agent pipeline
this skill originally described (e.g. the code-testing-generator pipeline) is not
reproduced as separate agents on Kiro; the skills it references (e.g.
`code-testing-agent`, `detect-static-dependencies`, `test-gap-analysis`) are
siblings in this same power and load on description match or via `disclose_context`.
EOF
    fi
}
...
# tag (e.g. v0.2.22) or the default branch (the bump workflow's path), so there is
