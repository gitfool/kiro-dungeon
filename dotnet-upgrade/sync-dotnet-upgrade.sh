...
# sync-dotnet-upgrade.sh — Mirrors the dotnet/skills dotnet-upgrade plugin skills
# from GitHub into this power's skills/ directory.
#
# Downloads the dotnet-upgrade plugin from the dotnet/skills repo and mirrors its
# skills into ./skills/, so this directory can be installed as a Kiro power.
#
# Unlike the sibling dotnet-msbuild power, dotnet-upgrade ships NO agents and NO
# MCP server: its plugin.json declares only skills. So this sync is purely a skill
# mirror — there is no agent-to-skill conversion and no mcp.json to maintain.
#
...
# Plugins 1.0.0 (missing $schema, and carries a skills key Kiro strips).
# See https://github.com/dotnet/skills/issues/1087. The .claude-plugin/,
# .codex-plugin/, and version.json entries live outside skills/, so they are
# naturally excluded.
#
# Support directories (references) under a skill are mirrored as-is. Only
...
#
# Usage:
#   sync-dotnet-upgrade.sh [--dry-run] [--ref <branch|tag|sha>]
...
BASE_PATH="plugins/dotnet-upgrade/skills"
# Path within the upstream repo whose commit date drives this power's version.
# The whole plugin dir, not just skills/, so a change to other plugin content is
# still reflected in the version.
UPSTREAM_PATH="plugins/dotnet-upgrade"
...
echo -e "${GREEN}sync-dotnet-upgrade: $REPOSITORY@$REF ($BASE_PATH)${RESET}"
...
# --- Orphan detection ---
# A local skill is an orphan if it maps to no upstream skill.
ORPHANS=()
...
# tag (e.g. v0.1.1) or the default branch (the bump workflow's path), so there is one
