...
# sync-aotw-dotnet-skills.sh — Mirrors the Aaronontheweb/dotnet-skills plugin
# skills and agents from GitHub into this power's skills/ directory.
#
# Downloads the dotnet-skills plugin from the Aaronontheweb/dotnet-skills repo and
# mirrors its skills into ./skills/, so this directory can be installed as a Kiro
# power.
#
# Additionally, the plugin's agent definitions (agents/*.md) are converted to
...
# frontmatter (user-invokable, tools) and normalizes the name, keeping the rest so
# the skill's activation intent is preserved. All six agents are substantial
# standalone specialists (not routing stubs), so all are converted; their names
# (akka-net-specialist, docfx-specialist, dotnet-benchmark-designer,
# dotnet-concurrency-specialist, dotnet-performance-analyst,
# roslyn-incremental-generator-specialist) are already descriptive, so — unlike
# the dotnet-msbuild power's "msbuild" agent — no rename is needed.
#
# GOTCHA: upstream agent files carry a DOUBLED frontmatter delimiter — the file
# opens with an empty "---\n---" block before the real "name:/description:" block.
# The awk converter skips that leading empty block, then rewrites the real one; a
# naive per-"---" counter (as in the dotnet-diag sibling) would mangle these.
#
# The agents carry editor/host tool references (task, web_fetch, web_search) that
...
# upstream adds such references later). This power ships NO MCP server, so the note
...
# Upstream's .claude-plugin/plugin.json is intentionally NOT mirrored: it is
# non-compliant with Agent Plugins 1.0.0 (missing $schema, and carries
# skills/agents keys Kiro strips). See
# https://github.com/dotnet/skills/issues/1087. The .claude-plugin/, .codex-plugin/,
# .agents/, scripts/, and top-level docs live outside skills/ and agents/, so they
# are naturally excluded.
#
# Support directories (references, scripts) and sibling reference .md files under a
# skill are mirrored as-is. Only immediate subdirectories of skills/ are treated as
# skills by Kiro, so nested markdown is never loaded and needs no special handling.
#
# This is a maintainer tool, not an installer. Run it to refresh skills/ when
# upstream changes, then commit the result.
#
# Usage:
#   sync-aotw-dotnet-skills.sh [--dry-run] [--ref <branch|tag|sha>]
...
# Empty means the whole repo (skills and agents both live at the repo root, not
# under a plugins/<name>/ subtree), so any change to skills or agents bumps the
# version. latest_commit_for_path treats an empty path as no path filter.
UPSTREAM_PATH=""
# Default ref when --ref is not given (the bump workflow's path). Upstream's
# default branch is "master", not "main".
REF="${REF:-master}"

# Agent basenames (without the .md suffix) to convert to skills. All six upstream
# agents are substantial standalone specialists, not routing stubs, so all are
# converted.
AGENTS_TO_CONVERT=(
    akka-net-specialist
    docfx-specialist
...
# Agent -> skill directory renames. None needed: every agent name is already
# descriptive and specific enough to sit alongside the other skills.
...
echo -e "${GREEN}sync-aotw-dotnet-skills: $REPOSITORY@$REF ($BASE_PATH)${RESET}"
...
# Find extracted root (GitHub archives have a top-level folder like dotnet-skills-main/)
...
# normalization is needed; SKILL.md and any sibling reference .md files or
# references/ dirs are copied verbatim.
sync_skill_folder() {
    local source_dir="$1"
    local skill_name="$2"
...
# body.
#
# Handles the upstream DOUBLED frontmatter delimiter: files open with an empty
# "---\n---" block before the real "name:/description:" block. The awk drops that
# leading empty block, then rewrites the real frontmatter. Delimiter accounting:
#   seen == 0, blank leading block: first "---" with the next line also "---" is
#   the empty opener — both are dropped. Otherwise "---" lines bound the real
#   frontmatter (fm 0 -> 1 -> 2). Inside fm == 1 we rewrite name and drop
#   agent-only keys.
convert_agent_to_skill() {
    ...
    awk -v name="$skill_name" '
        BEGIN { fm = 0; dropped_empty = 0 }
        # Drop a single leading empty frontmatter block ("---" immediately
        # followed by "---" before any frontmatter content has been seen).
        NR == 1 && /^---[[:space:]]*$/ { first = 1; next }
        first == 1 {
            first = 0
            if ($0 ~ /^---[[:space:]]*$/ && dropped_empty == 0) {
                # Second consecutive "---": the empty block closes here. Emit a
                # single opening delimiter for the real frontmatter that follows.
                dropped_empty = 1
                fm = 1
                print "---"
                next
            }
            # Not a doubled delimiter: the first "---" was the real opener.
            fm = 1
            print "---"
            # fall through to handle the current line as frontmatter content
        }
        /^---[[:space:]]*$/ { fm++; print; next }
        fm == 1 && /^name:/ { print "name: " name; next }
        fm == 1 && /^user-invokable:/ { next }
        fm == 1 && /^user-invocable:/ { next }
        fm == 1 && /^tools:/ { next }
        { print }
